...
const DashboardMainpage = () => {
  ...
  return (
    <div className="px-10 flex justify-center items-center">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-8 w-full max-w-3xl mx-auto px-4">
        {services?.map((service: any, index: number) => (
          ...
          >
            <h3 className="font-semibold text-blue-900 mb-4 text-2xl text-center">
              {service.title}
            </h3>
            ...
            {service.offer && (
              <div className="text-center mb-4">
                <span className="text-red-500 font-bold">{service.offer.highlight}</span>
                <div className="text-gray-800 text-lg font-bold mt-2">
                  Just <span className="line-through text-gray-500">{service.offer.originalPrice}</span>{" "}
            ...
            {service.earnings && (
              <div className="text-center mb-4">
                <span className="text-red-500 font-bold">{service.earnings.highlight}</span>
                <div className="text-gray-800 text-lg font-bold mt-2">
                  min <span className="text-green-500">{service.earnings.minEarnings}</span>{" "}
                  - <span className="text-green-500">{service.earnings.maxEarnings}</span> per day
                </div>
              </div>
            )}
            <a
              href={service.buttonLink}
              className="block w-full text-center font-bold py-2 rounded-lg transition-colors duration-300 bg-blue-900 text-white hover:bg-blue-600"
            >
              {service.buttonText}
            </a>
          </div>
