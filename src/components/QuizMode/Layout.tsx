    ...
    <div className="min-h-screen p-1">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-4 flex items-center">
          ...
          />
          <button
            className="ml-2 px-4 py-2 bg-white text-blue rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mt-8">
          {loading ? (
            ...
            filteredCategories.map((category, index) => (
              <div
                key={index}
                className="relative bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 transform hover:scale-105"
              >
                {/* Ribbon Tag */}
                <div
                  className={`absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 w-12 h-6 bg-${
                    category.type === "gov" ? "green-500" : "blue-500"
                  } text-white text-center leading-6 rounded-bl-lg`}
                >
                  {category.type === "gov" ? (
